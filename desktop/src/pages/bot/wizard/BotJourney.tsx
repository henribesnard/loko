...
export function BotJourney({ config, updateConfig, saving }: WizardStepProps) {
  const { t } = useTranslation();
  const [journey, setJourney] = useState<JourneyParams>({ ...config.journey });
  const [dirty, setDirty] = useState(false);

  const handleChange = (key: keyof JourneyParams, raw: string, isFloat: boolean) => {
